...
def SE_ResNeXt(input, class_dim, infer=False):
    ...
    conv = conv_bn_layer(
        input=input, num_filters=64, filter_size=(3, 3), stride=2, act="relu")
    conv = conv_bn_layer(
...
def train(learning_rate,
    ...
    train_reader = paddle.batch(train_data(), batch_size=batch_size)
    test_reader = paddle.batch(test_data(), batch_size=batch_size)
