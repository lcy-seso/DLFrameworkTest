...
def parse_all_reduce_spec(all_reduce_spec):
    ...
    if len(range_parts) % 2:
        raise ValueError("all_reduce_spec not well formed: %s" %
                         all_reduce_spec)
    limit = 0
    spec = []
    alg = None
...
def sum_grad_and_var_all_reduce(grad_and_vars,
    ...
    with tf.name_scope("allreduce"):
        ...
        if alg == "nccl":
            ...
            summed_grads = all_reduce.build_nccl_then_ring(scaled_grads,
                                                           num_shards, tf.add)
        elif alg == "nccl/rechd":
            ...
            summed_grads = all_reduce.build_shuffle_then_shuffle(
                scaled_grads, aux_devices, [aux_devices[0]], tf.add_n)
...
def sum_gradients_all_reduce(dev_prefixes,
    ...
    if "pscpu" in alg:
        ...
        aux_devices = [
            prefix + "/gpu:%d" % i
            for i in range(len(gpu_indices)) for prefix in dev_prefixes
        ]
    else:
    ...
    for grad_and_vars in zip(*tower_grads):
        reduced_gv_list.append(
            sum_grad_and_var_all_reduce(
                ...
                if is_hierarchical else aux_device_groups[
                    group_index], num_shards))
