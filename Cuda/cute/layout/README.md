...
通过上面几个例子，我们可以用下面这种方式，直觉地去理解一个Layout的“补”这样一个抽象运算要完成的事情。我们需要理解这样极简事情：
...
1. 第三步，用被除数`a`与`v2`进行函数的合成，结果v，就是除法的结果。由于v是由合成操作得到的一个Layout，`a`的domain就是`v`的domain，`v2`的range就是`v`的range。
