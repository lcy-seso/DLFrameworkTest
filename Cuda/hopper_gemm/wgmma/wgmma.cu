...
struct KeTraits {
  ...
  // global to shared copy using cp.async
  using CopyA = decltype(make_tiled_copy(
      Copy_Atom<SM80_CP_ASYNC_CACHEALWAYS<uint128_t>, DType>{},
      Layout<Shape<_16, _8>, Stride<_8, _1>>{},  // 128 threads
      Layout<Shape<_1, _8>>{}));
  ...
  // about the permutation layout (the third parameter of `TiledMMA`) see
  // this issue: https://github.com/NVIDIA/cutlass/discussions/1345
  using TiledMma = TiledMMA<
      MMA_Atom<SM90_64x64x16_F16F16F16_SS<GMMA::Major::K, GMMA::Major::K>>,
      Layout<Shape<_1, _1, _1>>, Tile<_64, _64, _16>>;

  static constexpr int kP = 3;  // pipeline stage

  // swizzle function: <3, 4, 3>, shape: ((8, 64), (64, 1))
  using SmemLayoutAtom = GMMA::Layout_K_SW128_Atom<DType>;

  using SmemLayoutA = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(Int<kTM>{}, Int<kTK>{}, Int<kP>{})));

  using SmemLayoutB = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(Int<kTN>{}, Int<kTK>{}, Int<kP>{})));
  ...
  using ProblemShape = Shape<Int<kM>, Int<kN>, Int<kK>>;
  using CtaTiler = Shape<Int<kTM>, Int<kTN>, Int<kTK>>;
};

template <typename DType, const int kM, const int kN, const int kK>
void hopper_gemm(const DType* gA, const DType* gB, DType* gC) {
  // Block shape and cta tiler
  constexpr int kTM = 64;
  constexpr int kTN = 64;
  constexpr int kTK = 64;

  using KeTraits = KeTraits<DType, kM, kN, kK, kTM, kTN, kTK>;

  using TiledMma = typename KeTraits::TiledMma;

  using CopyA = typename KeTraits::CopyA;
  CopyA copyA;

  using CopyB = typename KeTraits::CopyB;
  CopyB copyB;

  void const* kernel = reinterpret_cast<void const*>(
      &ke_cute_wgmma<DType, KeTraits, CopyA, CopyB>);

  constexpr int smem_size = KeTraits::kSmemSize;
  ...
  cutlass::Status status = cutlass::launch_kernel_on_cluster(
      launch_params, kernel, copyA, copyB, gA, gB, gC);
  CUTE_CHECK_LAST();
