...
__global__ void ke_cute_hopper_gemm(__grid_constant__ TmaLoadA const tma_load_A,
  ...
  constexpr int kM = KernelTraits::kM;
  constexpr int kN = KernelTraits::kN;
  constexpr int kK = KernelTraits::kK;

  constexpr int kBlockM = KernelTraits::kBlockM;
  constexpr int kBlockN = KernelTraits::kBlockN;
  constexpr int kBlockK = KernelTraits::kBlockK;
  ...
  //   auto cta_tiler = make_shape(Int<kBlockM>{}, Int<kBlockN>{},
  //   Int<kBlockK>{}); auto cta_coord = make_coord(blockIdx.y, blockIdx.x, _);
  ...
  //   Tensor sA =
  //       make_tensor(make_smem_ptr(shared_storage.smem_A.data()),
  //       SmemLayoutA{});
  //   Tensor sB =
  //       make_tensor(make_smem_ptr(shared_storage.smem_B.data()),
  //       SmemLayoutB{});

  //   auto [tAgA, tAsA] =
  //       tma_partition(tma_load_A, Int<0>{}, Layout<_1>{}, group_modes<0,
  //       2>(sA),
  //                     group_modes<0, 2>(gA));
  //   auto [tBgB, tBsB] =
  //       tma_partition(tma_load_B, Int<0>{}, Layout<_1>{}, group_modes<0,
  //       2>(sB),
  //                     group_modes<0, 2>(gB));
